...
### Enunciado:
> *"Se desea comparar dos procesadores que ejecutan la misma tarea. El procesador CISC requiere $2 \times 10^{18}\text{ instrucciones}$ con un CPI promedio de $4$ y una frecuencia de $2\text{ GHz}$. Por otro lado, el procesador RISC necesita más instrucciones ($5 \times 10^{18}$), pero opera con un CPI promedio de $1.5$ y una frecuencia de $3\text{ GHz}$. ¿Cuál procesador es más rápido y cuál es su factor de aceleración?"*

### 4.1. Identificación de Datos:
* **Procesador CISC:**
  * $I_{\text{CISC}} = 2 \times 10^{18}\text{ instrucciones}$
  * $\text{CPI}_{\text{CISC}} = 4$
  * $f_{\text{CISC}} = 2\text{ GHz} = 2 \times 10^9\text{ Hz}$
* **Procesador RISC:**
  * $I_{\text{RISC}} = 5 \times 10^{18}\text{ instrucciones}$
...
$$\text{Tiempo CPU}_{\text{CISC}} = \frac{I_{\text{CISC}} \times \text{CPI}_{\text{CISC}}}{f_{\text{CISC}}} = \frac{(2 \times 10^{18}) \times 4}{2 \times 10^9\text{ Hz}} = \frac{8 \times 10^{18}}{2 \times 10^9} = \mathbf{4 \times 10^9\text{ segundos}}$$

#### B) Tiempo de CPU del Procesador RISC:
$$\text{Tiempo CPU}_{\text{RISC}} = \frac{I_{\text{RISC}} \times \text{CPI}_{\text{RISC}}}{f_{\text{RISC}}} = \frac{(5 \times 10^{18}) \times 1.5}{3 \times 10^9\text{ Hz}} = \frac{7.5 \times 10^{18}}{3 \times 10^9} = \mathbf{2.5 \times 10^9\text{ segundos}}$$

---

### 4.3. Comparativa y Factor $n$ (Speedup):
Como $\text{Tiempo RISC} < \text{Tiempo CISC}$ ($2.5 \times 10^9\text{ s} < 4.0 \times 10^9\text{ s}$), **el procesador RISC es más rápido**.

* **Factor de Aceleración ($n$):**
  $$n = \frac{T_{\text{CISC}}}{T_{\text{RISC}}} = \frac{4.0 \times 10^9\text{ s}}{2.5 \times 10^9\text{ s}} = \frac{4.0}{2.5} = \mathbf{1.60\times}$$
  El procesador RISC es **$1.6$ veces más rápido** que el CISC *(las potencias de diez se cancelan exactamente)*.

* **Porcentaje de Mejora de Rendimiento (Fórmula Directa):**
  $$\% \text{ Mejora} = \left(\frac{T_{\text{CISC}} - T_{\text{RISC}}}{T_{\text{RISC}}}\right) \times 100\% = \left(\frac{4.0 \times 10^9 - 2.5 \times 10^9}{2.5 \times 10^9}\right) \times 100\% = \frac{1.5}{2.5} \times 100\% = \mathbf{60.0\%}$$

* **Porcentaje de Reducción de Tiempo:**
  $$\% \text{ Reducción} = \left(\frac{4.0 \times 10^9 - 2.5 \times 10^9}{4.0 \times 10^9}\right) \times 100\% = \frac{1.5}{4.0} \times 100\% = \mathbf{37.5\%}$$

> [!NOTE]
> **Lección Arquitectónica Clásica (CISC vs. RISC):**  
> Aunque RISC requiere un mayor número de instrucciones para la misma tarea ($5 \times 10^{18}$ frente a $2 \times 10^{18}$), su ventaja de tener instrucciones simples con **CPI reducido ($1.5$)** combinada con una **mayor frecuencia de reloj ($3\text{ GHz}$)** compensa holgadamente el volumen de código, resultando en una ejecución un **$60\%$ más rápida**.

---

## 5. Ejercicio 5: Comparación entre Computadoras X e Y con Instrucciones Distractoras

### Enunciado:
> *"Se tiene un programa en una computadora $X$ que tarda $35\text{ segundos}$ en ejecutarse, mientras que la computadora $Y$ tarda $21\text{ segundos}$. Se sabe que el programa está formado por $522\text{ millones de instrucciones}$. ¿Cuántas veces es más rápida la computadora $Y$ que la computadora $X$?"*

### 5.1. Identificación de Datos:
* $T_X = 35\text{ s}$
* $T_Y = 21\text{ s}$
* $I = 522\text{ millones de instrucciones} = 522 \times 10^6\text{ instrucciones}$

> [!TIP]
> **Análisis del Dato Distractor:**  
> Como el programa es idéntico en ambas computadoras, el número de instrucciones ($I = 522 \times 10^6$) es constante y se cancela algebraicamente. Para saber cuántas veces es más rápida una máquina sobre otra solo se requiere la razón de sus tiempos de ejecución.

---

### 5.2. Resolución Matemática (Factor $n$ / Speedup):
Fundamentado en la [[3. Relación de Rendimiento y Comparación de Máquinas (Factor n)]]:

$$n = \frac{\text{Rendimiento de } Y}{\text{Rendimiento de } X} = \frac{T_X}{T_Y}$$

Sustituyendo los tiempos y simplificando entre $7$:
$$n = \frac{35\text{ s}}{21\text{ s}} = \frac{35 \div 7}{21 \div 7} = \frac{5}{3} \approx \mathbf{1.6667} \approx \mathbf{1.67\times}$$

La computadora $Y$ es **$1.67$ veces más rápida** que la computadora $X$ (o $\frac{5}{3}\times$).

---

### 5.3. Métricas Complementarias:
* **Porcentaje de Mejora de Rendimiento (Fórmula Directa):**
  $$\% \text{ Mejora} = \left(\frac{T_X - T_Y}{T_Y}\right) \times 100\% = \left(\frac{35 - 21}{21}\right) \times 100\% = \frac{14}{21} \times 100\% = \frac{2}{3} \times 100\% \approx \mathbf{66.67\%}$$

* **Porcentaje de Reducción de Tiempo:**
  $$\% \text{ Reducción} = \left(\frac{T_X - T_Y}{T_X}\right) \times 100\% = \left(\frac{35 - 21}{35}\right) \times 100\% = \frac{14}{35} \times 100\% = \frac{2}{5} \times 100\% = \mathbf{40.0\%}$$

* **Comprobación vía Tasa de Procesamiento (MIPS):**
  * $\text{MIPS}_X = \frac{522 \times 10^6}{35 \times 10^6} = \frac{522}{35} \approx \mathbf{14.91\text{ MIPS}}$
  * $\text{MIPS}_Y = \frac{522 \times 10^6}{21 \times 10^6} = \frac{522}{21} \approx \mathbf{24.86\text{ MIPS}}$
  * Razón: $\frac{\text{MIPS}_Y}{\text{MIPS}_X} = \frac{24.86}{14.91} = \frac{35}{21} = \mathbf{1.67\times}$

---

## 6. Conexión con Throughput y la Fórmula de Eficiencia


Tal como se definó en la [[2. Medidas de Rendimiento y Concepto de Speedup#4. Tiempo de Respuesta vs. Throughput (Productividad)]], el rendimiento global de un sistema también se evalúa mediante la **tasa de procesamiento (Throughput)** frente a los límites físicos del hardware:
