...
## 7. Generación de Datos Sintéticos para Clases Minoritarias: SMOTE

Cuando una clase es ultra-minoritaria (como la Clase 1 con solo 210 puntos frente a 10,200), el submuestreo de la clase mayoritaria puede no ser suficiente. Para enriquecer a la minoría sin duplicar puntos idénticos (lo que causaría sobreajuste), se emplea **SMOTE (*Synthetic Minority Over-sampling Technique*)**.

### 7.1. Algoritmo Paso a Paso
1. **Selección del Punto Base:** Se selecciona un ejemplo $x_i$ perteneciente a la clase minoritaria en el espacio de características.
2. **Búsqueda de $k$ Vecinos Cercanos ($k$-NN):** Se calculan sus $k$ vecinos más cercanos (típicamente $k = 5$) que también pertenezcan a la misma clase minoritaria.
3. **Selección Aleatoria de un Vecino:** Se escoge aleatoriamente uno de esos $k$ vecinos, denotado como $\hat{x}_i$.
4. **Interpolación Lineal (Generación Sintética):** Se traza el segmento rectilíneo que une a $x_i$ con $\hat{x}_i$ y se genera un nuevo punto sintético en una posición intermedia aleatoria:

$$x_{\text{sintético}} = x_i + \lambda (\hat{x}_i - x_i) \quad \text{donde } \lambda \in [0, 1]$$

```mermaid
flowchart LR
    A["<b>Punto Minoritario (x_i)</b>"] -->|Segmento de Línea| B["<b>Vecino k-NN (x_hat)</b>"]
    A -.->|Interpolación λ ∈ [0, 1]| S["⭐ <b>Punto Sintético Nuevo</b>"]
    S -.-> B
```

---

### 7.2. Condición Crítica de Validez: Densidad Local vs. Datos Dispersos

> [!WARNING]
> **La Regla de Oro de SMOTE:**
> SMOTE **solo es válido si los datos de la clase minoritaria están agrupados o unidos en el espacio**. 
> - **Caso Favorable (Datos Unidos / Continuos):** Si los puntos minoritarios forman una franja o nube densa, cualquier punto interpolado entre ellos preserva la topología de la clase y refuerza su cobertura convexa.
> - **Caso Crítico (Datos Aislados o Ruidosos / Outliers):** Si los puntos están muy separados o aislados en medio de la clase mayoritaria, trazar una línea entre ellos generará puntos sintéticos en **territorio enemigo**, inyectando ruido severo y deformando las fronteras de decisión de la Red Neuronal.

---

## 📢 Avisos de Clase y Calendario
> [!NOTE]
> **Continuidad de Laboratorio:**
> El próximo **Viernes, 4 de septiembre de 2026** se concluirá formalmente el bloque temático sobre **Construcción de Muestras y Técnicas de Balanceo**.
